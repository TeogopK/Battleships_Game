...
class Button:
    def __init__(self, x, y, text, font_size, text_color, bg_color, hover_color, width, height, padding=10, border_radius=5):
        ...
        self.rect.topleft = (x + (width - self.rect.width) //
                             2, y + (height - self.rect.height) // 2)
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface, bg_color, self.button_rect,
                         border_radius=self.border_radius)

        pygame.draw.rect(surface, (0, 0, 0), self.button_rect,
                         2, border_radius=self.border_radius)

        text_x = self.button_rect.x + \
            (self.button_rect.width - self.image.get_width()) // 2
        text_y = self.button_rect.y + \
